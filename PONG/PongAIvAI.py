...
def directions_from_input(paddle_rect, other_paddle_rect, ball_rect, table_size):
    ...
    try:
        ...
        value= gyro_sender.get_rotation_degree("x")
...
def directions_from_input2(paddle_rect, other_paddle_rect, ball_rect, table_size):
    try:

        value= gyro_sender2.get_rotation_degree("x")
...
def testFunction(paddle_rect, other_paddle_rect, ball_rect, table_size):
    keys = pygame.key.get_pressed()
...
def render(screen, paddles, ball, score, table_size):
    ...
    pygame.draw.rect(screen, red, paddles[0].frect.get_rect())  #changed color from white
...
def init_game():
    table_size = (1200, 900)
    #1900 by 1000
    paddle_size = (10, 250) #changed from 130
    ball_size = (25, 25)
    paddle_speed = 10
    max_angle = 30 #changed from 45
    ...
    paddles[1].move_getter = directions_from_input2
